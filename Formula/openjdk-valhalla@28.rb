class OpenjdkValhallaAT28 < Formula
  desc "JDK 28 early-access OpenJDK build with Project Valhalla value objects"
  ...
  livecheck do
    skip "pinned to an adversarially reviewed Valhalla lworld commit"
  end
  ...
  def install
    ...
    args = %W[
      ...
      --with-version-opt=valhalla-f181286389fa
      --with-version-pre=ea
  ...
  def caveats
    <<~EOS
      For the system Java wrappers to find this JDK, symlink it with
        sudo ln -sfn #{opt_libexec}/openjdk.jdk /Library/Java/JavaVirtualMachines/openjdk-valhalla-28.jdk
    EOS
  end
  ...
  test do
    ...
    system bin/"javac", "--enable-preview", "--release", "28", "ValueSmoke.java"
    output = shell_output("#{bin}/java --enable-preview ValueSmoke")
    assert_match "equal = true", output
    assert_match "identity = false", output
    assert_match "valhalla-f181286389fa", shell_output("#{bin}/java --version 2>&1")
  end
end
